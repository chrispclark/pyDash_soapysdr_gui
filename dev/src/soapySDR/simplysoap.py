...
class SoapyDevice:
    ...
    def channel(self, channel):
        ...
        if channel in self.list_channels():
            ...
            logger.warning(
                "Incorrect RX channel number, using channel 0 instead!"
            )
            self._channel = 0
    ...
    def freq(self, freq):
        ...
        if freq < freq_range[0] or freq > freq_range[1]:
            raise ValueError(
                "Center frequency out of range ({}, {})!".format(
    ...
    def sample_rate(self, sample_rate):
        ...
        if self.force_sample_rate:
            ...
            if sample_rate != real_sample_rate:
                logger.warning(
                    "Sample rate {} Hz is not supported, setting it to {} Hz!".format(
                        sample_rate, real_sample_rate
                    )
                )

        self._sample_rate = real_sample_rate
        self.device.setSampleRate(
            SoapySDR.SOAPY_SDR_RX, self._channel, real_sample_rate
        )
    ...
    def bandwidth(self, bandwidth):
        ...
        if self.force_bandwidth:
            ...
            if not band_ranges:
                logger.warning(
                    "Device does not support setting filter bandwidth!"
                )
                return
            ...
            if bandwidth != real_bandwidth:
                logger.warning(
                    "Filter bandwidth {} Hz is not supported, setting it to {} Hz!".format(
                        bandwidth, real_bandwidth
                    )
                )

        self._bandwidth = real_bandwidth
        self.device.setBandwidth(
            SoapySDR.SOAPY_SDR_RX, self._channel, real_bandwidth
        )
    ...
    def auto_gain(self, auto_gain):
        ...
        self.device.setGainMode(
            SoapySDR.SOAPY_SDR_RX, self._channel, auto_gain
        )
    ...
    def corr(self):
        ...
        try:
            return self.device.getFrequency(
                SoapySDR.SOAPY_SDR_RX, self._channel, "CORR"
            )
        except RuntimeError:
            return 0
    ...
    def corr(self, corr):
        ...
        if corr < corr_range[0] or corr > corr_range[1]:
            raise ValueError(
                "Frequency correction out of range ({}, {})!".format(
                    corr_range[0], corr_range[1]
                )
            )

        self._corr = corr
        self.device.setFrequency(
            SoapySDR.SOAPY_SDR_RX, self._channel, "CORR", corr
        )
    ...
    def list_sample_rates(self):
        ...
        try:
            rate_ranges = Ranges(
                (f.minimum(), f.maximum())
                for f in self.device.getSampleRateRange(
        ...
        if rate_ranges:
            ...
            rate_list = self.device.listSampleRates(
                SoapySDR.SOAPY_SDR_RX, self._channel
            )
    ...
    def list_bandwidths(self):
        ...
        try:
            band_ranges = Ranges(
                (f.minimum(), f.maximum())
                for f in self.device.getBandwidthRange(
                    SoapySDR.SOAPY_SDR_RX, self._channel
                )
            )
        except AttributeError:
        ...
        if band_ranges:
            ...
            band_list = self.device.listBandwidths(
                SoapySDR.SOAPY_SDR_RX, self._channel
            )
    ...
    def list_frequencies(self):
        ...
        return self.device.listFrequencies(
            SoapySDR.SOAPY_SDR_RX, self._channel
        )
    ...
    def list_stream_args(self):
        ...
        args = {
            ...
            for a in self.device.getStreamArgsInfo(
                SoapySDR.SOAPY_SDR_RX, self._channel
            )
        }
        return args
    ...
    def get_gain(self, amp_name):
        ...
        return self.device.getGain(
            SoapySDR.SOAPY_SDR_RX, self._channel, amp_name
        )
    ...
    def set_gain(self, amp_name, value):
        ...
        if amp_name not in self.list_gains():
            raise ValueError("Unknown amplification element!")
        self.device.setGain(
            SoapySDR.SOAPY_SDR_RX, self._channel, amp_name, value
        )
    ...
    def get_frequency(self, tunable_name):
        ...
        return self.device.getFrequency(
            SoapySDR.SOAPY_SDR_RX, self._channel, tunable_name
        )
    ...
    def set_frequency(self, tunable_name, value):
        ...
        if tunable_name not in self.list_frequencies():
            raise ValueError("Unknown tunable element!")
        self.device.setFrequency(
            SoapySDR.SOAPY_SDR_RX, self._channel, tunable_name, value
        )
    ...
    def get_frequency_range(self, tunable_name=None):
        ...
        if tunable_name:
            if tunable_name not in self.list_frequencies():
                raise ValueError("Unknown tunable element!")
            freq = self.device.getFrequencyRange(
                SoapySDR.SOAPY_SDR_RX, self._channel, tunable_name
            )[0]
        else:
            freq = self.device.getFrequencyRange(
    ...
    def start_stream(self, buffer_size=0, stream_args=None, stream_timeout=0):
        ...
        if not buffer_size:
            try:
                ...
                logger.warning(
                    "getStreamMTU not implemented! Using default value: {}".format(
    ...
    def stop_stream(self):
        ...
        if not self.is_streaming:
            raise RuntimeError(
                "Streaming is not initialized, you must run start_stream() first!"
            )
    ...
    def read_stream(self, stream_timeout=0):
        ...
        if res.ret > 0 and res.ret < buffer_size:
            logger.warning(
                "readStream returned only {} samples, but buffer size is {}!".format(
                    res.ret, buffer_size
                )
            )
        return res
    ...
    def read_stream_into_buffer(self, output_buffer):
        ...
        while True:
            ...
            if res.ret > 0:
                output_buffer[ptr : ptr + res.ret] = self.buffer[
                    : min(res.ret, output_buffer_size - ptr)
                ]
                ptr += res.ret
            elif res.ret == -4:
...
if __name__ == "__main__":
    ...
    z: SoapyDevice = SoapyDevice()
    logger.info(z.list_channels())
    logger.info(z.hardware)
    logger.info(detect_devices(as_string=True))
    sdr = SoapyDevice("driver=rtlsdr")
    logger.info(sdr)
    b = sdr.list_antennas()
    logger.info(b)
    b = sdr.list_channels()
    logger.info(f"channel {b}")

    a = sdr.list_antennas()
    logger.info(f"antennas {a}")
    b = sdr.list_frequencies()
    logger.info(f"frequencies {b}")
    a = z.list_bandwidths()
    logger.info(f"bandwidth {a}")
    a = z.bandwidth
    logger.info(f"bandwidth {a}")
    a = z.list_sample_rates()
    logger.info(f"sample_rates {a}")
    a = z.get_setting()
    logger.info(f"get settings {a}")
    a = z.list_settings()
    logger.info(f"settings {a}")
    a = z.get_gain_range()
    logger.info(f"gain range {a}")
    a = z.channel()
    logger.info(f"channel {a}")
    a = z.list_gains()
    logger.info(f"gain {a}")
    a = z.list_stream_args()
    logger.info(f"stream args {a}")

    sdr.sample_rate = 2.56e6
    c = sdr.set_frequency("RF", 97.3e6)
    a = sdr.get_frequency("RF")
    logger.info(a)
    c = sdr.start_stream()
    logger.info(c)

    def callback(outdata: numpy.ndarray, frames: int, time, status):
    ...
    samples = numpy.empty(len(sdr.buffer) * 1, numpy.complex64)
    while True:
        # logger.info(samples)

        # Receive all samples
        sdr.read_stream_into_buffer(samples)
        samples = samplesQ.put()
        logger.info(samples)
        # decimate 1/5 from 1.2MHz to 240kHz
        sigif = scipy.signal.decimate(samples, 5, ftype="iir")
